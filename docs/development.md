...
VLM training (any run with `[model.vlm]` set, SFT or RL) requires a registered VLM architecture and an explicitly supported packed multimodal forward path. Qwen3-VL uses its Hugging Face image-to-text model directly; families that need a Prime-specific model should extend a custom text model with a composite VLM body — the Qwen3.5 dense (`models/qwen3_5/`) and MoE (`models/qwen3_5_moe/`) implementations are the reference. The pieces, in dependency order:
...
5. **Registration.** Describe every supported family in `VLM_REGISTRY` (`utils/vlm.py`). Register composite custom models in `_CUSTOM_VLM_MAPPING` (`models/__init__.py`) so `get_model` dispatches to the custom class; a Hugging Face-backed family must instead be explicitly allowlisted only after its packed multimodal forward path is validated.
